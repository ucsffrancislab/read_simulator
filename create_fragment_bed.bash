...
function usage(){
	...
	echo
	echo "$script [--reference STRING] [--min_length INTEGER] [--max_length INTEGER] [--outbase STRING]"
...
#	echo "  distance .. : $distance (for overlappers)"
	...
	echo
	echo "Example:"
	echo "$script --reference hg38-mhc.fa --length 1000 --outbase illumina"
	echo
	exit
}
...
while read -r refchr reflength a b c ; do
	...
	if [ -f $f ] && [ ! -w $f ] ; then
		...
		for i in $( seq -w ${chr_count} ) ; do
			...
			echo "${refchr}	${cstart}	${cend}	${refchr}_seq${i}" >> $f
